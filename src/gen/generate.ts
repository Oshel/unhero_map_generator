...
/**
 * Fill in walkable pockets that no exit can reach. A style can easily leave an
 * island behind - a walled-off cell, a cave chamber the automaton cut loose -
 * and an island is dead space: the player never sees it, but the game still
 * decorates and spawns around it.
 */
...
function buildOnce(
  ...
  return {
    doc: {
      ...
      exits,
      // A doorway that the pocket pass walled off is no longer a doorway.
      doors: doors.filter((d) => layers.blocking[d.y][d.x] === 'void'),
      layers,
      markers,
    },
    resolved,
